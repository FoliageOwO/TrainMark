...
@Component
@ConditionalOnProperty(name = "trainmark.course.store", havingValue = "memory", matchIfMissing = true)
public class InMemoryCourseStore implements CourseStore {
  ...
  public InMemoryCourseStore() {
    ...
    assignments.put(1L, new AssignmentSummary(1L, 1L, "Java Web 综合实训报告", OffsetDateTime.now().plusDays(7), 100, AssignmentStatus.PUBLISHED, true, true));
  }
